...
class audioloop:
    ...
    def initialize(self):
        ...
        # crossfade
        # fade out the last recorded buffer
    ...
    def dub(self, data, fade_in=False, fade_out=False):
        ...
        self.audio[self.writep, :] = self.audio[self.writep, :] * \
            0.9 + datadump[:] * self.dub_ratio
...
def set_recording(loop_number=0):
    ...
    # then set_recording() was called to finish the recording, i.e. new audio just got added.
...
def restart_looper():
    ...
    # replaces current process with a new instance of the same script
